...
export GFB_DOMAINS=${SERVER_URLS// /,}
...
for name in MARIADB_ENV_MARIADB_DATABASE MARIADB_ENV_MARIADB_USER MARIADB_ENV_MARIADB_PASSWORD MARIADB_PORT_3306_TCP_ADDR MARIADB_PORT_3306_TCP_PORT AWS_ACCESS_KEY AWS_SECRET_ACCESS_KEY NGINX_USER NGINX_PATH_PREFIX SERVER_URLS GFB_DOMAINS LETSENCRYPT_WEB_HOME WP_HOME MY_USER REDIS_PORT_6379_TCP_ADDR REDIS_PORT_6379_TCP_PORT WP_HOST_IP GCS_MEDIA_BUCKET GCS_MEDIA_MODE GCS_MEDIA_KEY_FILE_PATH GCS_MEDIA_SERVICE_ACCOUNT
do
    eval value=\$$name;
    sudo sed -i "s|\${${name}}|${value}|g" /etc/letsencrypt/configs/globalfoodbook.com.conf;
...
# if [[ ! -d /home/$MY_USER/app/letsencrypt/.well-known ]]; then
...
sudo chown -R $NGINX_USER:$NGINX_USER $LETSENCRYPT_WEB_HOME > /dev/null 2>&1 &
